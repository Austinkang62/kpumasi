 ...
 * 아바타 자동 생성 Cron Job
 *
 * 매 5분마다 실행되어 Avatar Points >= $100인 회원의 아바타를 자동 생성
 *
 * Cron 설정 예시:
 * */5 * * * * /usr/bin/php /ai22/www/admin/cron-avatar-generator.php >> /ai22/www/admin/logs/avatar-cron.log 2>&1
 */
...
if (php_sapi_name() !== 'cli') {
    // 웹에서 접근 시에도 실행 가능하도록 (테스트용)
    if (!isset($_GET['key']) || $_GET['key'] !== 'avatar_cron_2024') {
        die('Access denied. CLI only or use ?key=avatar_cron_2024');
    }
}
...
try {
        ...
        for ($i = 0; $i < $avatarCount; $i++) {
            try {
                ...
                $db->execute("
                    ...
                    VALUES (?, 'avatar_purchase', 100.00, 'AVATAR_POINTS', ?, 'Avatar 자동 생성 (Cron)', ?)
    ...
    logMessage("===== 완료 =====");
    logMessage("생성된 아바타: {$totalCreated}개");
    logMessage("실패: {$totalFailed}개");
