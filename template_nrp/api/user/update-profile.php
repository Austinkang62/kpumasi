...
try {
    ...
    // 필수 필드 체크
    if (empty($name) || empty($email) || empty($phone) || empty($usdtAddress)) {
        http_response_code(400);
        echo json_encode([
            'success' => false,
    ...
    // USDT 주소 형식 검증 (0x + 40자의 16진수)
    if (!preg_match('/^0x[a-fA-F0-9]{40}$/', $usdtAddress)) {
        http_response_code(400);
        echo json_encode([
            'success' => false,
            'message' => 'USDT 주소 형식이 올바르지 않습니다.'
        ]);
        exit;
    }
    ...
    try {
        ...
        $db->insert($logQuery, [
            ...
            json_encode([
                'updated_fields' => ['name', 'email', 'phone', 'usdt_address'],
