...
logMessage("생성 대기: " . count($users) . "명");
...
foreach ($users as $user) {
    ...
    logMessage("회원 {$userIdStr} 처리 중 (포인트: \${$avatarPoints}, 생성: {$avatarCount}개)");
    ...
    for ($i = 0; $i < $avatarCount; $i++) {
        try {
            ...
            $db->execute("
                ...
                VALUES (?, 2, 100.00, 'AVATAR_AUTO', 'AVATAR_MANUAL', 'completed', ?, ?)
            ...
            $db->execute("
                ...
                VALUES (?, 'avatar_purchase', 100.00, 'AVATAR_POINTS', ?, 'Avatar 수동생성', ?)
            ", [$avatarInfo['avatar_internal_id'], $saleId, $salesDateTime]);

            $db->commit();

            $totalCreated++;
            logMessage("  ✅ {$avatarInfo['avatar_user_id']} 생성 완료 (보너스 {$bonusResult['count']}건)");

        } catch (Exception $e) {
            $db->rollback();
            $totalFailed++;
            logMessage("  ❌ 실패: " . $e->getMessage());
        }
    }
}

logMessage("========== 완료 ==========");
logMessage("✅ 성공: {$totalCreated}개 | ❌ 실패: {$totalFailed}개");
?>
