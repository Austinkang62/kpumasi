...
try {
    ...
    if ($method === 'POST' && $action === 'create') {
        ...
        try {
            ...
            // 5. 아바타 포인트 체크 및 자동 생성
            // (커밋 후 별도 트랜잭션으로 처리)
            $avatarsCreated = [];
            try {
                $avatarsCreated = checkAndCreateAvatars($db);
            } catch (Exception $e) {
                error_log('Avatar auto-creation error: ' . $e->getMessage());
                // 아바타 생성 실패해도 매출 입력은 성공으로 처리
            }
            ...
            echo json_encode([
                ...
                'avatars_created' => count($avatarsCreated),
                'avatar_details' => $avatarsCreated
            ]);
            ob_end_flush();
