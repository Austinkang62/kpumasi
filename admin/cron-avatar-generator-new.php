<?php
/**
 * 아바타 자동 생성 - Cron Job 버전
 *
 * 실행 URL: https://ai22.mycafe24.com/admin/cron-avatar-generator-new.php?key=avatar_cron_2024
 *
 * Cafe24 Cron 설정: */5 * * * * /usr/bin/curl -s "https://ai22.mycafe24.com/admin/cron-avatar-generator-new.php?key=avatar_cron_2024" > /dev/null 2>&1
 */
...
logMessage("========== 아바타 자동 생성 Cron Job 시작 ==========");
...
if (empty($eligibleUsers)) {
    logMessage("생성 대기 중인 아바타가 없습니다.");
    logMessage("========== Cron Job 종료 ==========");
    exit;
}

logMessage("생성 대기 중인 회원: " . count($eligibleUsers) . "명");
...
foreach ($eligibleUsers as $user) {
    ...
    logMessage("회원 {$userIdStr} 처리 시작 (Avatar Points: \${$avatarPoints}, 생성 예정: {$avatarCount}개)");
    ...
    for ($i = 0; $i < $avatarCount; $i++) {
        try {
            ...
            $db->execute("
                ...
                VALUES (?, 2, 100.00, 'AVATAR_AUTO', 'AVATAR_CRON', 'completed', ?, ?)
            ...
            $db->execute("
                ...
                VALUES (?, 'avatar_purchase', 100.00, 'AVATAR_POINTS', ?, 'Avatar Cron 자동생성', ?)
            ", [$avatarInfo['avatar_internal_id'], $saleId, $salesDateTime]);

            $db->commit();

            $totalCreated++;
            logMessage("  ✅ 아바타 {$avatarInfo['avatar_user_id']} 생성 완료 (보너스 {$bonusResult['count']}건 지급)");

        } catch (Exception $e) {
            $db->rollback();
            logMessage("  ❌ 아바타 생성 실패: " . $e->getMessage());
        }
    }

    logMessage("회원 {$userIdStr} 처리 완료");
}

logMessage("========== Cron Job 완료 ==========");
logMessage("생성된 아바타 총 개수: {$totalCreated}개");
?>
